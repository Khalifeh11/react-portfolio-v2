const subheadingsData = {
  1: [
    {
      title: "About Me",
      content: (
        <>
          <p>
            Multidisciplinary learner passionate about leveraging new
            technologies to develop user-friendly web and mobile applications.
            Collaborative team player with a focus on creating intuitive digital
            experiences.
          </p>
        </>
      ),
    },
  ],
  2: [
    {
      title: "2018 Bangor University",
      content: (
        <>
          <p>
            <li>MSc in Clinical and Health Psychology</li>
          </p>
        </>
      ),
    },
    {
      title: "2016 Lebanese American University",
      content: (
        <>
          <p>
            <li>BA in Psychology</li>
          </p>
        </>
      ),
    },
  ],
  3: [
    {
      title: "2022 Storiad",
      content: (
        <>
          <p>
            <li>Software Developer</li>
          </p>
        </>
      ),
    },
    {
      title: "2021 SE Factory",
      content: (
        <>
          <p>
            <li>FullStack Web Development Internship</li>
          </p>
        </>
      ),
    },
    {
      title: "2020 Saint George Hospital",
      content: (
        <>
          <p>
            <li>
              Clinical Psychologist
            </li>
          </p>
        </>
      ),
    },
  ],
};

export default subheadingsData;
